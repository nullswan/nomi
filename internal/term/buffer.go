...
// From: https://github.com/ollama/ollama/blob/main/readline/buffer.go
	...
	if b.Pos < b.Buf.Size() {
		if e, ok := b.Buf.Get(b.Pos); ok {
			if r, ok := e.(rune); ok {
				...
				if b.DisplayPos%b.LineWidth == 0 {
					fmt.Print(
						CursorDown + CursorBOL + CursorRightN(
							len(b.Prompt.prompt()),
						),
					)
...
func (b *Buffer) MoveToEnd() {
	if b.Pos < b.Buf.Size() {
		...
		if currLine < totalLines {
			...
			fmt.Print(
				CursorBOL + CursorRightN(len(b.Prompt.prompt())+remainder),
			)
		} else {
...
func (b *Buffer) drawRemaining() {
	...
	if (b.DisplayPos+currLineSpace)%b.LineWidth == 0 &&
		currLine == remainingText {
	...
	if remLength > currLineSpace {
		...
		for _, c := range remaining {
			if displayLength == 0 ||
				(displayLength+runewidth.RuneWidth(c))%b.LineWidth < displayLength%b.LineWidth {
				fmt.Printf("\n%s", b.Prompt.AltPrompt)
				totalLines += 1
				...
				if displayLength != 0 {
					if lineLength == b.LineWidth {
						b.LineHasSpace.Set(
							b.DisplayPos/b.LineWidth+totalLines-1,
							false,
						)
					} else {
		...
		fmt.Print(
			ClearToEOL + CursorUpN(
				totalLines,
			) + CursorBOL + CursorRightN(
				b.Width-currLineSpace,
			),
		)
		...
		if e, ok := b.Buf.Get(b.Pos - 1); ok {
			if r, ok := e.(rune); ok {
				...
				if b.DisplayPos%b.LineWidth == 0 {
					...
					fmt.Print(
						CursorBOL + ClearToEOL + CursorUp + CursorBOL + CursorRightN(
							b.Width,
						),
					)
				...
				if (b.DisplaySize()-1)%b.LineWidth == 0 ||
